...
function GameApp() {
  ...
  const finishRun = useCallback(() => {
    ...
    setShowContinuePrompt(!continuedRunRef.current);
  ...
  return <LinearGradient colors={['#F9F0B7', '#BCEACD']} style={styles.fill}><StatusBar style="dark" /><ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.homeContent}>
    <View style={styles.homeTop}><Pill><Text style={styles.pillText}>🌰 {save.acorns}</Text></Pill><View style={styles.homeTopRight}><Pill dark><Text style={[styles.pillText, { color: '#FFF' }]}>BEST {save.bestScore}</Text></Pill><Pressable accessibilityLabel="Settings" onPress={() => setScreen('settings')} style={styles.homeSettingsButton}><Text style={styles.homeSettingsIcon}>⚙️</Text></Pressable></View></View>
    ...
    <View style={[styles.heroStage,{backgroundColor:environment.colors[1]}]}><Text style={styles.sun}>{save.selectedWorld==='volcano'?'🌋':save.selectedWorld==='snow'?'❄️':'☀️'}</Text><Text style={styles.heroPlant}>{environment.decor.split('  ')[0]}</Text><Text style={styles.heroPlantTwo}>{environment.decor.split('  ')[1]}</Text><View style={styles.heroOutfitPill}><Text style={styles.heroOutfitName}>{selectedOutfit.name}</Text><Text style={styles.heroOutfitPerk}>{selectedPerk?.label ?? 'Classic balanced hopping'}</Text></View><Capybara outfit={selectedOutfit} size={150} jumping /><Pressable onPress={()=>setScreen('missions')} style={styles.homeWorldPill}><Text style={styles.homeWorldText}>{environment.name} • Change{featuredWorld===save.selectedWorld?' • +50% 🌰':''}</Text></Pressable></View>
    <Pressable onPress={openGame} style={styles.primaryButton}><Text style={styles.primaryButtonText}>PLAY</Text></Pressable>
    <View style={styles.homeQuickGrid}><Pressable onPress={() => setScreen('shop')} style={styles.homeQuickButton}><Text style={styles.homeQuickEmoji}>👒</Text><Text style={styles.homeQuickText}>Closet</Text></Pressable><Pressable onPress={() => setScreen('missions')} style={styles.homeQuickButton}><Text style={styles.homeQuickEmoji}>🎁</Text><Text style={styles.homeQuickText}>Rewards</Text></Pressable><Pressable onPress={() => setScreen('missions')} style={styles.homeQuickButton}><Text style={styles.homeQuickEmoji}>🎯</Text><Text style={styles.homeQuickText}>Quests</Text></Pressable><Pressable onPress={() => setScreen('leaderboard')} style={styles.homeQuickButton}><Text style={styles.homeQuickEmoji}>🏆</Text><Text style={styles.homeQuickText}>Ranks</Text></Pressable></View>
    ...
    <Text style={styles.homeSectionLabel}>YOUR ADVENTURE</Text>
    <View style={styles.homeAdventureRow}><Pressable accessibilityRole="button" onPress={() => setScreen('missions')} style={styles.homeAdventureCard}><Text style={styles.homeAdventureEmoji}>🎯</Text><Text style={styles.nextWorldText}>WEEKLY CHALLENGES</Text><Text style={styles.worldMapHint}>375 acorns available</Text></Pressable><Pressable accessibilityRole="button" onPress={() => setScreen('missions')} style={styles.homeAdventureCard}><Text style={styles.homeAdventureEmoji}>🗺️</Text><Text style={styles.nextWorldText}>PASSPORT</Text><Text style={styles.worldMapHint}>{save.progress.journeyObstacles} cleared • {JOURNEY_REWARDS.filter(tier => save.progress.journeyObstacles >= tier.goal && !save.progress.journeyClaimed.includes(tier.goal)).length} ready</Text></Pressable></View>
...
const styles = StyleSheet.create({capyFrame:{position:'absolute',width:'100%',height:'100%'},highScorePop:{backgroundColor:'#F6B83D',borderRadius:22,paddingHorizontal:18,paddingVertical:10,alignItems:'center'},highScorePopText:{color:'#FFFDF0',fontSize:22,fontWeight:'900',letterSpacing:1,textAlign:'center'},highScorePopSub:{color:'#FFF5B8',fontSize:10,fontWeight:'900',letterSpacing:2,marginTop:2},resultsLabel:{color:'#71806D',fontSize:10,fontWeight:'900',letterSpacing:3},
  fill:{flex:1},homeContent:{flexGrow:1,paddingHorizontal:20,paddingTop:20,paddingBottom:34,alignItems:'center',gap:14},homeTop:{width:'100%',maxWidth:380,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},homeTopRight:{flexDirection:'row',alignItems:'center',gap:9},homeSettingsButton:{width:42,height:42,borderRadius:99,backgroundColor:'#FFF9E5',alignItems:'center',justifyContent:'center',boxShadow:'0 4px 0 rgba(77,79,39,.14)'},homeSettingsIcon:{fontSize:20},pill:{backgroundColor:'#FFF9E5',borderRadius:99,paddingHorizontal:16,paddingVertical:9,boxShadow:'0 4px 0 rgba(77,79,39,.14)'},pillText:{color:'#39513B',fontWeight:'900',fontSize:14,fontVariant:['tabular-nums']},logoWrap:{alignItems:'center',gap:0},buildBadge:{color:'#FFFFFF',backgroundColor:'#4E8055',fontSize:10,fontWeight:'900',letterSpacing:1.4,paddingHorizontal:12,paddingVertical:5,borderRadius:99,overflow:'hidden',marginBottom:5},logoSmall:{color:'#527441',fontSize:17,fontWeight:'900',letterSpacing:6},logoBig:{color:'#F27D4C',fontSize:58,lineHeight:60,fontWeight:'900',letterSpacing:-2,textShadowColor:'#FFF8D8',textShadowOffset:{width:0,height:4},textShadowRadius:0},tagline:{color:'#57705C',fontSize:12,fontWeight:'700',letterSpacing:1.1},heroStage:{height:230,width:'100%',maxWidth:380,alignItems:'center',justifyContent:'center',backgroundColor:'rgba(255,255,255,.28)',borderRadius:34,borderCurve:'continuous',overflow:'hidden',boxShadow:'0 8px 20px rgba(70,100,66,.12)'},sun:{position:'absolute',right:23,top:15,fontSize:38},heroPlant:{position:'absolute',left:18,bottom:12,fontSize:50},heroPlantTwo:{position:'absolute',right:18,bottom:15,fontSize:32},heroOutfitPill:{position:'absolute',left:14,top:14,maxWidth:'55%',backgroundColor:'rgba(255,249,229,.9)',borderRadius:14,paddingHorizontal:10,paddingVertical:7,zIndex:2},heroOutfitName:{color:'#3E5B42',fontSize:11,fontWeight:'900'},heroOutfitPerk:{color:'#A16535',fontSize:8,fontWeight:'800',paddingTop:1},homeWorldPill:{position:'absolute',bottom:10,alignSelf:'center',backgroundColor:'rgba(48,75,56,.9)',borderRadius:99,paddingHorizontal:14,paddingVertical:7},homeWorldText:{color:'#FFF',fontWeight:'900',fontSize:11},homeQuickGrid:{width:'100%',maxWidth:380,flexDirection:'row',gap:8},homeQuickButton:{flex:1,minHeight:70,backgroundColor:'#FFF9E8',borderRadius:18,borderCurve:'continuous',alignItems:'center',justifyContent:'center',gap:3,borderBottomWidth:3,borderBottomColor:'#D7CA94'},homeQuickEmoji:{fontSize:23},homeQuickText:{color:'#3F583F',fontSize:10,fontWeight:'900'},homeSectionLabel:{width:'100%',maxWidth:380,color:'#58705D',fontSize:11,fontWeight:'900',letterSpacing:1.6,paddingTop:5},homeAdventureRow:{width:'100%',maxWidth:380,flexDirection:'row',gap:9},homeAdventureCard:{flex:1,minHeight:96,backgroundColor:'rgba(255,255,255,.58)',borderRadius:20,borderCurve:'continuous',padding:12,gap:4},homeAdventureEmoji:{fontSize:22},
